...
void stopMotors() {
  // Writes "0" to motors to stop them
  analogWrite(pwmA, 0);
  analogWrite(pwmB, 0);
...
bool moveForwardUntilBump(float distance_cm, int speed) {
  /*
  Function activates both motors to move forwards until it the bottom limit switch hits the collection zone, and then it stops
  */
  digitalWrite(dirA, HIGH);
  digitalWrite(dirB, HIGH);
...
void setArmPosition(int angle1, int angle2) {
  // Constrain servo angles to preven it from trying to go further than it actually can
